...
export const analyzeForFiveMeldsNoKnitted : HandAnalyzer<MeldBasedWinningHand> = (hand: Hand) => {
    ...
    .filter(melds => meldsAreSubset(melds, hand.userSpecifiedMelds))
    .map(melds => {
        ...
        // the melds can be the same but it could matter in scoring for some special win conditions. (like plum blossom on the roof)
        return melds.map((meld, index) => {
            ...
            // this meld has the most recent tile and is currently marked as unexposed by default
            // mostRecentTileUserSpecifiedMeld is undefined, but hand.mostRecentTileIsSelfDrawn could have been set manually by the user.
            if (!hand.mostRecentTileIsSelfDrawn()) {
                // if mostRecentTileUserSpecifiedMeld was not specified, but the recent tile is specified to have been eaten via discard, 
