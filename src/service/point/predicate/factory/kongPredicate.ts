...
export function createKongEqualityPredicate(pointPredicateID : string, tiles: SuitedOrHonorTile[]) : PointPredicate<StandardWinningHand> {
    return (winningHand : StandardWinningHand) => {
        ...
        const intersection : Meld[] = meldsIntersection(winningHand.getContents(), kongs, true);
        return new PointPredicateResult(pointPredicateID, intersection.length == kongs.length, intersection);
    }
}

export function createKongQuantityPredicate(pointPredicateID : string, numKongs: number) : PointPredicate<StandardWinningHand> {
    return (winningHand : StandardWinningHand) => {
        const kongs : Kong[] = [];
        ...
        return new PointPredicateResult(pointPredicateID, kongs.length === numKongs, kongs);
    }
}